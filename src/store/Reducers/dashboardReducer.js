...
export const get_user_dashboard_data = createAsyncThunk(
    'dashboard/get_user_dashboard_data',
    async( _ ,{rejectWithValue, fulfillWithValue}) => { 
        try {
            const {data} = await api.get('/user/get-dashboard-data',{withCredentials: true})             
            return fulfillWithValue(data)
        } catch (error) { 
            return rejectWithValue(error.response.data)
        }
...
export const dashboardReducer = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        .addCase(get_user_dashboard_data.fulfilled, (state, { payload }) => {
            state.totalSale = payload.totalSale
            state.totalOrder = payload.totalOrder
            state.totalProduct = payload.totalProduct 
